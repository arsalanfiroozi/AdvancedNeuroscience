...
V = 0.01; % Measure variance
P = 0.5; % Process variance
w = [0; 0];
sig = [0.6 0; 0 0.6];
n_Trials = 10;
...
legend({'W1', 'W2'});
box off
title(['Measurement Noise: ' num2str(V) ', Process Noise: ' num2str(P)]);
subplot(2,1,2);
...
ylabel('Sigma');
legend({'S1', 'S2'});
export_fig(['V' num2str(V) 'P' num2str(P) 'Kalman_Blocking.png'],'-r600');
%% Q2
% Fig 1 Unblocking
clearvars
...
ylabel('Sigma');
legend({'S1', 'S2'});
export_fig(['V' num2str(V) 'P' num2str(P) 'Kalman_Unblocking.png'],'-r600');
%% Q2
...
axis square
title({['Measurement Noise: ' num2str(V) ', Process Noise:' num2str(P)] 't = 9'});
subplot(1,3,3);
w1 = -1:0.02:2;
w2 = -1:0.02:2;
[w1,w2] = meshgrid(w1,w2);
...
axis square
export_fig(['V' num2str(V) 'P' num2str(P) 'Kalman_Fig2.png'],'-r600');